function varargout =  Compute_Sulcal_Top_Bottom_Lines(varargin);
%
% Syntax :
...
for planes = 1:Np
    try
        [intercLine] = Intercept_Plane_with_Surface([n1(planes,:) D1(planes,:)],Surf2Process); % Interception between each plane and sulcal surface
    catch
        intercLine = '';
    end
...
% ind = find(topCurv>mean(topCurv)+2*std(topCurv));
% while ~isempty(ind)
%     bottomline(ind,:) = [];
%     [~,~,~,topCurv,~] = frenet(bottomline(:,1),bottomline(:,2),bottomline(:,3));
%     ind = find(topCurv>mean(topCurv)+2*std(topCurv));
