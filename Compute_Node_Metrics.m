function varargout = Compute_Node_Metrics(varargin);
%
% Syntax :
...
[sulchullIntercep, sulchullIntNormals] = Intercept_Surface_with_Surface(HullSurfMat,  Surf2Process);
while length(unique(sulchullIntercep(:,4)))~=1
    HullSurfMat.SurfData.vertices = HullSurfMat.SurfData.vertices - HullSurfMat.SurfData.VertexNormals*0.5;
    [sulchullIntercep,sulchullIntNormals] = Intercept_Surface_with_Surface(HullSurfMat,  Surf2Process);
end
...
%            intNormals = [xtempNormal(:) ytempNormal(:) ztempNormal(:)];
           ...
           tempVar = normm(intNormals);
           intNormals = intNormals./[tempVar tempVar tempVar];
