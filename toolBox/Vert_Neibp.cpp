    ...
    for (int j = 0; j < Nf; j++)
        {
            ...
            Tri[(int)(posc2)] = faces[(int)(j+Nf)];
            Tri[(int)(posc)] = Tri[(int)(posc)]+1;
            posc2 = posc+Tri[(int)(posc)]*Nv;
            Tri[(int)(posc2)] = faces[(int)(j+2*Nf)];
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    // Return Matrices:
    plhs[0] = mxCreateDoubleMatrix((int)Nv[0], 300, mxREAL); //crea la matriz inicializada en cero
    Tri = mxGetPr(plhs[0]);
